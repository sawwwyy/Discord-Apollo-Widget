...
2. Ensure you have the correct label IDs
   - You will need the following keys:
     a. `total_objekts` (tripleS objekt count)
     b. `triples_como` (tripleS COMO balance)
     c. `first_class_objekts` (First class objekts)
     d. `double_class_objekts` (Double class objekts)
     e. `special_class_objekts` (Special class objekts)
     f. `premier_class_objekts` (Premier class objekts)
